...
def customer_identity_health_payload() -> Dict[str, Any]:
    ...
    _log_config_status(config)
    return {
        "customer_own_company_aliases_configured": config.configured,
        "customer_own_company_aliases_valid": config.valid,
        "customer_own_company_aliases_count": _config_alias_count(config),
    }
...
def customer_identity_prompt_context(org_id: str) -> str:
    ...
    return (
        ...
        "甲方和乙方只是合同标签，不能据此直接判断买卖角色；若一方命中己方别名，另一方才是客户候选。"
    )

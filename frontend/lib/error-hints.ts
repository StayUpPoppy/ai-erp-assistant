type CategoryKey = "master_data" | "permission" | "timeout" | "upstream_error" | "default";
...
function parseMapFromEnv(raw?: string): Partial<Record<CategoryKey, string>> {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(parsed)) {
      ...
      if (k === "master_data" || k === "permission" || k === "timeout" || k === "upstream_error" || k === "default") {
        output[k] = v;
      }
    }
    return output;
...
function toCategoryKey(category?: string): CategoryKey {
  if (category === "master_data" || category === "permission" || category === "timeout" || category === "upstream_error") {
    return category;
  }
  return "default";
}
